...
function buildProxyUrl(origin, source) {
    ...
    // If a Rust proxy sidecar is running, point all media URLs there.
    // The Rust proxy speaks the same token format — no other changes needed.
    const rustProxy = process.env.STREAM_PROXY_URL;
    if (rustProxy) {
        return `${rustProxy.replace(/\/$/, "")}/api/proxy?t=${encodeURIComponent(token)}`;
    }
