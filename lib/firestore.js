...
export async function getUserData(uid) {
  ...
  return {
    ...data,
    continueWatching: dedupeContinueWatching(data.continueWatching),
  };
}

// ── Continue Watching ─────────────────────────────────────────────────────────

/** Keep at most one entry per animeId (most-recent first). */
function dedupeContinueWatching(entries) {
  const seen = new Set();
  return (entries ?? []).filter((e) => {
    if (!e.animeId || seen.has(e.animeId)) return false;
    seen.add(e.animeId);
    return true;
  });
}
 ...
 * One record per animeId — replaces any prior entry for the same show.
...
export async function upsertContinueWatching(uid, entry) {
  ...
  const existing = dedupeContinueWatching(data.continueWatching ?? []).filter(
...
/**
 * Remove an anime from continueWatching (e.g. on completion).
 */
export async function removeContinueWatching(uid, animeId) {
  const ref  = userRef(uid);
  const snap = await ref.get();
